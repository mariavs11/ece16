...
class Pedometer:
  ...
  def __init__(self, num_samples, fs, data=None):
    ...
    self.__b, self.__a = filt.create_filter(3, 1.7, "lowpass", fs)
  ...
  def process(self):
    ...
    # Filter the signal (detrend, low pass)
    # Detrend
    dt = filt.detrend(x)
    # Low-Pass Filtering
...
class JumpingJacksCounter:
  ...
  __b = None         # Low-pass coefficients
  __a = None         # Low-pass coefficients
  __thresh_low = 800   # Threshold
  __thresh_high = 2000 # Threshold
